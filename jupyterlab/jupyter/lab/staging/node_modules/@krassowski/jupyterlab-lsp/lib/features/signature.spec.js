...
describe('Signature', () => {
    describe('extractLead', () => {
        it('Extracts standalone one-line paragraph', () => {
            const split = extractLead(['This function does foo', '', 'But there are more details'], 1);
        ...
        it('Does not extracts when it would break markdown', () => {
            let split = extractLead(['This is **not the end', '', 'of this spread sentence**'], 1);
            expect(split).to.equal(null);
            split = extractLead(['This is <b>not the end', '', 'of this spread sentence</b>'], 1);
            expect(split).to.equal(null);
        });
    ...
    describe('SignatureToMarkdown', () => {
        const MockHighlighter = (code, fragment) => code.replace(fragment, `<u>${fragment}</u>`);
        it('renders plaintext signature', async () => {
            let text = signatureToMarkdown({
                ...
                activeParameter: 0
            }, 'python', MockHighlighter, new BrowserConsole());
            expect(text).to.be.equal('str(<u>text</u>)\n\nCreate a new \\*string\\* object from the given object.\n');
        });
        ...
        it('renders plaintext with details and paramaters', async () => {
            let text = signatureToMarkdown({
                ...
                activeParameter: 0
            }, 'python', MockHighlighter, new BrowserConsole(), undefined, 4);
            expect(text).to.be.equal('str(<u>text</u>)\n\nline 1\n<details>\nline 2\nline 3\nline 4\nline 5\n</details>');
        });
        it('renders plaintext with details and no parameters', async () => {
            ...
            expect(text).to.be.equal('```python\nstr()\n```\n\nline 1\n<details>\nline 2\nline 3\nline 4\nline 5\n</details>');
        });
    });
});
