...
import { FileEditorFeatureTestEnvironment, MockSettings, NotebookFeatureTestEnvironment, code_cell, set_notebook_content, showAllCells } from '../../editor_integration/testutils';
...
describe('Diagnostics', () => {
    let feature;
    let default_settings = new MockSettings({
    ...
    describe('FileEditor integration', () => {
        ...
        it('filters out inspections by code', async () => {
            ...
            expect(markers[0].title).to.equal('Undefined symbol "aa"');
        });
        it('filters out inspections by message text', async () => {
            ...
            let markers = env.ce_editor.editor.getDoc().getAllMarks();
            expect(markers.length).to.equal(1);
            expect(markers[0].title).to.equal('Trimming whitespace');
        });
    });
    describe('Notebook integration', () => {
        ...
        it('renders inspections across cells', async () => {
            ...
            let cm_editors = env.adapter.editors.map(editor => editor.editor);
        ...
        it('Works in foreign documents', async () => {
            ...
            let foreign_feature = env.init_integration({
                constructor: DiagnosticsCM,
                id: 'Diagnostics',
            ...
            let cm_editors = env.adapter.editors.map(ce_editor => ce_editor.editor);
            let marks_cell_1 = cm_editors[0].getDoc().getAllMarks();
            let marks_cell_2 = cm_editors[1].getDoc().getAllMarks();
