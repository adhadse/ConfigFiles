...
describe('LSPCompleterModel', () => {
    let model;
    function create_dummy_item(match, type = 'dummy') {
        return new LazyCompletionItem(type, null, match, null, 'file://test.ipynb');
    }
    ...
    it('returns escaped when no query', () => {
        ...
        expect(markedItems[0].label).to.be.equal('&lt;i class="jp-icon-jupyter"&gt;&lt;/i&gt; Jupyter');
    });
    it('marks html correctly', () => {
        model.setCompletionItems([jupyter_icon_completion]);
        model.query = 'Jup';
        let markedItems = model.completionItems();
        expect(markedItems[0].label).to.be.equal('&lt;i class="jp-icon-jupyter"&gt;&lt;/i&gt; <mark>Jup</mark>yter');
    });
    ...
    it('marks appropriate part of label when filterText matches', () => {
        ...
        expect(markedItems[0].label).to.be.equal('&lt;<mark>i</mark> class="jp-icon-jupyter"&gt;&lt;/i&gt; Jupyter');
    });
});
